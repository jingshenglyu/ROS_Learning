...
void voiceWordsCallback(const std_msgs::String::ConstPtr& msg)
{
	...
	{
		char front_text[100]="我在向前啦";
		text=front_text;
	}
	else if(dataString.find("后")!=std::string::npos)
	{
		char back_text[100]="我在向后啦";
		text=back_text;
	}
	else if(dataString.find("左")!=std::string::npos)
	{
		char left_text[100]="正在向左，主人";
		text=left_text;
	}
	else if(dataString.find("右")!=std::string::npos)
	{
		char right_text[100]="正在向右，主人";
		text=right_text;
	}
	else if(dataString.find("停")!=std::string::npos 
		|| dataString.find("别动")!=std::string::npos)
	{
		char stop_text[100]="我没动";
		text=stop_text;
	}
	else if(dataString.find("小l")!=std::string::npos)
	{
		char name_text[100]="你好，我是小L，你的机器人控制语音智能";
		text=name_text;
	}
	else
	{
...
{
	int         ret                  = MSP_SUCCESS;
	const char* login_params         = "appid = 5e99b5ff, work_dir = .";//登录参数,appid与msc库绑定,请勿随意改动
	/*
	* rdn:           合成音频数字发音方式
	* volume:        合成音频的音量
	* pitch:         合成音频的音调
